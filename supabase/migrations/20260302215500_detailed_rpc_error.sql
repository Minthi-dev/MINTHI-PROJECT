-- Migration to fix the error message in register_restaurant_secure
-- So we can debug what is actually causing the "already exists" error.

CREATE OR REPLACE FUNCTION public.register_restaurant_secure(
    p_name text,
    p_phone text,
    p_email text,
    p_username text,
    p_password_hash text,
    p_raw_password text
) RETURNS jsonb AS $$
DECLARE
    new_restaurant_id uuid;
    new_user_id uuid;
    result jsonb;
BEGIN
    -- 1. Insert into users first to get the owner id
    INSERT INTO public.users (email, name, username, password_hash, raw_password, role)
    VALUES (p_email, p_username, p_username, p_password_hash, p_raw_password, 'OWNER')
    RETURNING id INTO new_user_id;

    -- 2. Insert into restaurants with the owner_id
    INSERT INTO public.restaurants (name, phone, is_active, owner_id)
    VALUES (p_name, p_phone, false, new_user_id)
    RETURNING id INTO new_restaurant_id;

    -- 3. Prepare result
    result := jsonb_build_object(
        'restaurant_id', new_restaurant_id,
        'user_id', new_user_id
    );

    RETURN result;
EXCEPTION
    WHEN unique_violation THEN
        -- Include the actual error message indicating what was duplicated
        RAISE EXCEPTION 'Esiste già un utente o un ristorante con questi dati. Dettaglio tecnico: %', SQLERRM;
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Errore durante la registrazione: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
