...
serve(async (req) => {
    ...
    try {
        ...
        // For update/delete without restaurantId, look it up
        let resolvedRestaurantId = restaurantId;
        if (!resolvedRestaurantId && targetId) {
            const { data: table } = await supabase.from("tables").select("restaurant_id").eq("id", targetId).maybeSingle();
            resolvedRestaurantId = table?.restaurant_id;
        }

        if (!resolvedRestaurantId) return json({ error: "restaurantId non determinabile" }, 400);

        // Verify owner or admin (staff cannot manage tables)
        ...
        switch (action) {
            case "create": {
                ...
                const { data: result, error } = await supabase.from("tables").insert(data).select().single();
                if (error) return json({ error: error.message }, 500);
                return json({ success: true, data: result });
            }
            case "update": {
                const id = targetId;
                if (!id || !data) return json({ error: "targetId e data richiesti" }, 400);
        ...
        console.error("[SECURE-TABLE] Errore:", error);
